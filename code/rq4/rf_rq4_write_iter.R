...
modelBuilding <- function(preprocess_data,class,training_data){
  ...
  statistic_fun <- function(data,indices){
    ...
    d <- data[indices,]
    fit <-randomForest(form,d,ntree = 500,importance = TRUE)
    importance_result <- importance(fit,type=1) 
    return(importance_result)
  }
  # bootstrap 1000 iterations, R=1000
  boot_result <- boot(data = training_data,statistic = statistic_fun,R=100)
  #extract the importance
  importance_variable <- boot_result$t
  #add the metric name to the importance variable
  colnames(importance_variable) <- metric_names
  return(importance_variable)
}
...
mainrun <- function(dataFile,commitBoundaryFile,evaluate_file,class_list) {
  ...
  for(class in class_all)
  {
    ...
    for(row in 2:nrow(commitBoundary))
    {
      ...
      sk <- sk_esd(importance_variable)
      #bind this rank of this iteration to the whole data frame
      all_metric_ranks <-bind_rows(all_metric_ranks,sk$groups)
    }
    #replace NA with 0
...
commitBoundaryFile <- "cassandra_boundary.txt"
#output file #1- "cassandraEvaluate.csv" 
#2- "hadoopEvaluate.csv"
#3- "openjpaEvaluate.csv"
evaluate_file <- "write/cassandraEvaluate.csv"
#class label
